...
const PageTemplate = () => {
  return (
    <>
      ...
      {/* MOBILE */}
      <div className="relative grid h-full grid-cols-12 grid-rows-9 max-xs:h-screen max-xs:w-screen max-xs:grid-cols-6 max-xs:grid-rows-4 max-xs:flex-col max-xs:gap-0 md:hidden">
        <div className="relative z-30 border-solid border-goldLines max-md:border-r-4 max-xs:col-start-1 max-xs:col-end-2 max-xs:row-span-4 max-xs:row-start-1 max-xs:w-1/2"></div>
        <div className="text-page-title-color relative z-20 flex items-center justify-center border-b-4 border-goldLines bg-primaryBackground bg-opacity-45 max-xs:col-span-6 max-xs:col-start-1 max-xs:row-start-1 max-xs:h-1/4 max-xs:text-base md:col-span-1 md:col-start-1 md:row-start-1">
          SERVICES
        </div>
        ...
        <div className="relative z-20 grid h-full  max-xs:col-span-5 max-xs:col-start-2 max-xs:row-span-4 max-xs:row-start-1 max-xs:h-3/4 max-xs:place-items-center md:col-start-3 md:col-end-11 md:row-start-3 md:row-end-7">
          <div className="relative flex flex-col gap-10 bg-primaryBackground bg-opacity-45 max-xs:py-4 max-xs:pl-2 max-xs:pr-4 md:h-full md:w-full md:justify-evenly md:gap-12">
            <div className="text-subtitle-color md:text-subtitle max-xs:text-other relative max-xs:text-left">
              Discover our comprehensive range of services
            </div>
            <div className="text-subtitle-description-color  max-xs:text-subtitle-description relative font-Fahkwang font-thin max-xs:text-left md:ml-48 md:text-xl">
              Discover our diverse expertise. From architectural design to
              construction, our talented team brings creativity and innovation
              to every project. Explore our areas of expertise and let us bring
              your vision to life.
